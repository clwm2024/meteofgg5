...
def signifikanztest_page(dfGrouped,dependent_vars):
    ...
    try:
        ...
        st.write("Korrelationsmatrix konnte nicht erstellt werden.")
...
if uploaded_file is not None:
    ...
    # Alle Spalten auswählen, die "cloud_cover" im Namen enthalten
    columns_to_aggregate = [col for col in filtered_df.columns if "cloud_cover" in col and "_diff" in col]
    ...
    # Erstelle ein Dictionary für die Aggregation, indem du nur die gefilterten Spalten verwendest
    agg_dict = {col: 'mean' for col in columns_to_aggregate}
    # Gruppiere und aggregiere nur die gefilterten Spalten
    dfDataGrouped = filtered_df.groupby(['locname', 'timediffofforecast']).agg(agg_dict).reset_index()
    ...
    measurements = columns_to_aggregate
    ...
    if page == "Plot":
        ...
        for i, location in enumerate(selected_locations):
            ...
            for measurement in selected_measurements:
                if measurement in columns_to_aggregate:
        ...
        ax.set_title('Liniendiagramm der Wolkendeckenunterschiede für gewählte Orte')
    ...
    if 'Signifikanztest I -' in page:
        ...
        agg_dict = {col: 'mean' for col in columns_to_aggregate}
        # Gruppiere und aggregiere nur die gefilterten Spalten
        dfDataGroupedTimediffOnly = filtered_df.groupby(['locname', 'timediffofforecast']).agg(agg_dict).reset_index()

        # Hier wird davon ausgegangen, dass du bereits ein dfGrouped hast oder es vorher lädst
        signifikanztest_page(dfDataGroupedTimediffOnly,columns_to_aggregate)
